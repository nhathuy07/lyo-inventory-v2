<script lang="ts">
    ...
    function calculate_restock_data(
        records: Record[],
        ...
        variant_by_id.forEach((variant, id) => {
            ...
            variant.on_hand = inventory?.on_hand || 0;
            variant.incoming = inventory?.incoming || 0;
            variant.available = inventory?.available || 0;
            ...
            // @ts-ignore
            if (
                variant.available + variant.incoming <=
                // @ts-ignore
                ...
                // @ts-ignore
                variant.restock = Math.round(sales_by_sku.get(variant.sku));
                variant.restock_half = Math.round(0.5 * variant.restock);
                variant.restock_third = Math.round((1 / 3) * variant.restock);
    ...
    function getLastDataUpdateTUnix() {
    ...
    function is_promotional_item(brand: string) {
        if (
    ...
    async function get_active_products() {
    ...
    async function fetch_order_record(variant_by_id: Map<number,ProductV2> ) {
                                        ...
                                        fulfillment.fulfillment_line_items.forEach(
                                            (line_item: any) => {
                                                ...
                                                if (line_item.is_composite) {
                                                    // TODO: break down composite items
                                                    const it = variant_by_id.get(line_item.variant_id)
                                                    // TODO: Get quantity and SKU of components inside the composite item
    ...
    async function fetchInventoryTransferFromIndexedDB(): Promise<
        TransferRecord[]
    > {
        return new Promise((resolve, reject) => {
            const request = indexedDB.open("LYOInventoryDB", 3);
    ...
    async function saveInventoryTransferToIndexedDB(
    ...
    async function fetch_inventory_transfer(
    ...
    onMount(async () => {
        ...
        let prods = await get_active_products();
        let x = await fetch_order_record(prods);
        let y = await fetch_inventory_transfer(prods);

        calculate_restock_data([...x, ...y], prods);
